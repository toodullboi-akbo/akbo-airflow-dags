...
def batter_daily_work(batter_ID_tuple : tuple, attempt : int):
    ...
    try:
        ...
        if attempt < MAX_RETRIES:
            ...
            batter_daily_work(batter_ID_tuple=batter_ID_tuple, attempt=attempt+1)
        else:
...
if __name__ == "__main__":
    try:
        ...
        for i in range(0, NUM_PROCESS):
            if i == NUM_PROCESS-1 : 
                process_list.append(Process(target=batter_daily_work, args=(batter_number_list[split_index*i:],1)))
            else : 
                process_list.append(Process(target=batter_daily_work, args=(batter_number_list[split_index*i:split_index*(i+1)],1)))
