...
def save_whole_year_team_pitcher_data():
    '''
    현재부터 TEAM_MIN_YEAR +1까지 팀 투수 기록 가져오기
    '''
    ...
    for year_idx in range(len(year_selector.options)-1,-1,-1):
        ...
        df['team_ERA'] = df['team_ERA'].astype(float)
        df['team_G'] = df['team_G'].astype(int)
        df['team_W'] = df['team_W'].astype(int)
        df['team_L'] = df['team_L'].astype(int)
        df['team_SV'] = df['team_SV'].astype(int)
        df['team_HLD'] = df['team_HLD'].astype(int)
        df['team_WPCT'] = df['team_WPCT'].astype(float)
        df['team_IP'] = df['team_IP'].astype(float)
        df['team_H'] = df['team_H'].astype(int)
        df['team_HR'] = df['team_HR'].astype(int)
        df['team_BB'] = df['team_BB'].astype(int)
        df['team_HBP'] = df['team_HBP'].astype(int)
        df['team_SO'] = df['team_SO'].astype(int)
        df['team_R'] = df['team_R'].astype(int)
        df['team_ER'] = df['team_ER'].astype(int)
        df['team_WHIP'] = df['team_WHIP'].astype(float)
        df['team_CG'] = df['team_CG'].astype(int)
        df['team_SHO'] = df['team_SHO'].astype(int)
        df['team_QS'] = df['team_QS'].astype(int)
        df['team_BSV'] = df['team_BSV'].astype(int)
        df['team_TBF'] = df['team_TBF'].astype(int)
        df['team_NP'] = df['team_NP'].astype(int)
        df['team_AVG'] = df['team_AVG'].astype(float)
        df['team_2B'] = df['team_2B'].astype(int)
        df['team_3B'] = df['team_3B'].astype(int)
        df['team_SAC'] = df['team_SAC'].astype(int)
        df['team_SF'] = df['team_SF'].astype(int)
        df['team_IBB'] = df['team_IBB'].astype(int)
        df['team_WP'] = df['team_WP'].astype(int)
        df['team_BK'] = df['team_BK'].astype(int)
...
if __name__ == "__main__":
    try :
        ...
        st_time = time.time()
        save_whole_year_team_pitcher_data()
