...
def save_whole_year_team_batter_data():
    '''
    현재부터 TEAM_MIN_YEAR +1까지 팀 타자 기록 가져오기
    '''
    ...
    for year_idx in range(len(year_selector.options)-1,-1,-1):
        ...
        df['team_PA'] = df['team_PA'].astype(int)
        df['team_AB'] = df['team_AB'].astype(int)
        ...
        df['team_TB'] = df['team_TB'].astype(int)
        df['team_RBI'] = df['team_RBI'].astype(int)
        ...
        df['team_GDP'] = df['team_GDP'].astype(int)
        df['team_SLG'] = df['team_SLG'].astype(float)
        df['team_OBP'] = df['team_OBP'].astype(float)
        df['team_OPS'] = df['team_OPS'].astype(float)
        df['team_MH'] = df['team_MH'].astype(int)
        df['team_RISP'] = df['team_RISP'].astype(float)
        df['team_PH_BA'] = df['team_PH_BA'].astype(float)

        save_df(
            df,
            os.path.join(DATASET_NAME,TEAM_DATASET_NAME,"batter",f"Team_Batter_{year}.parquet"),
...
if __name__ == "__main__":
    try :
        ...
        st_time = time.time()
        save_whole_year_team_batter_data()
