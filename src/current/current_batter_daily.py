...
def batter_daily_work(shared_number_list, index : int, attempt : int, driver, queue):
    '''
    ...
    try:
        ...
        if attempt < MAX_RETRIES:
            ...
            batter_daily_work(shared_number_list, index, attempt=attempt+1, driver=driver,queue=queue)
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(0, NUM_PROCESS):
            if i == NUM_PROCESS-1 : 
                process_list.append(Process(target=batter_daily_work, args=(shared_number_list[i],i,1,drivers[i],queue)))
            else : 
                process_list.append(Process(target=batter_daily_work, args=(shared_number_list[i],i,1,drivers[i],queue)))
